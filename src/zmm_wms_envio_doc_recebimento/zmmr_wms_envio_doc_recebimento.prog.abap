...
*&---------------------------------------------------------------------*
REPORT zmmr_wms_envio_doc_recebimento.
TABLES: ztmm_doc_rem_rec, t001.

DATA go_integracao TYPE REF TO zclmm_wms_integrar_recebimento.

SELECTION-SCREEN: BEGIN OF BLOCK b1 WITH FRAME TITLE TEXT-t01.
  SELECT-OPTIONS:
  s_vbeln  FOR ztmm_doc_rem_rec-vbeln,
  s_erdat  FOR ztmm_doc_rem_rec-erdat,
  s_ztipo  FOR ztmm_doc_rem_rec-ztipo_doc,
  s_status FOR ztmm_doc_rem_rec-zstatus_integracao,
  s_werks  FOR ztmm_doc_rem_rec-werks.
...
AT SELECTION-SCREEN OUTPUT.
  go_integracao = NEW zclmm_wms_integrar_recebimento( ).
...
START-OF-SELECTION.
  ...
  go_integracao->exec_integracao(
    EXPORTING
      ir_documento     = CORRESPONDING #( s_vbeln[] )
  ...
  TRY.
      ...
      go_columns->get_column( TEXT-c01 )->set_visible( abap_false ).
      go_columns->get_column( TEXT-c02 )->set_visible( abap_false ).
      go_columns->get_column( TEXT-c03 )->set_visible( abap_false ).
