CLASS zclmm_benef_fiscal_dpc_ext DEFINITION
  PUBLIC
  INHERITING FROM zclmm_benef_fiscal_dpc
  CREATE PUBLIC .
...
CLASS zclmm_benef_fiscal_dpc_ext IMPLEMENTATION.
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~create_stream.
    ...
    DATA(lo_benef_fiscal) = NEW zclmm_benef_fiscal( ).

    SPLIT iv_slug AT ';' INTO lv_filename lv_tablename.

    IF is_media_resource-mime_type <> lv_mime_type.
      ##NO_TEXT
      lt_return[] = VALUE #( BASE lt_return ( type = 'E' id = 'ZCA_EXCEL' number = '005' ) ).
    ELSE.
* ----------------------------------------------------------------------
* Gerencia Botão do aplicativo
* ----------------------------------------------------------------------
      lo_benef_fiscal->upload_file( EXPORTING iv_file      = is_media_resource-value
