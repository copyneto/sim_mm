...
***********************************************************************
***                                                                   *
*** DESCRIÇÃO: RFC Execução de Processos WMS - JOB                    *
...
***-------------------------------------------------------------------*
*** 15/11/2023| Jean Silva | Versão Inicial                           *
***********************************************************************

*&--------------------------------------------------------------------*
*& Função ZFMMM_EXEC_PROCE
*&--------------------------------------------------------------------*
*&
*&--------------------------------------------------------------------*
FUNCTION zfmmm_exec_proce.
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     VALUE(IT_R_VBELN) TYPE  LMD_VL_T_VBELN_VL_RANGE OPTIONAL
*"     VALUE(IT_R_TIPO_DOC) TYPE  ZCTGMM_RANGE_ZTIPO_DOC OPTIONAL
*"     VALUE(IV_STATUS_INT) TYPE  ZE_STATUS_INT OPTIONAL
*"  EXPORTING
*"     VALUE(ET_MESSAGES) TYPE  BAPIRET2_T
*"----------------------------------------------------------------------


  CONSTANTS lc_jobname TYPE btcjob VALUE 'ZMMR_EXE_PROC_WMS'.
  ...
  SUBMIT (lc_jobname)
    ...
    WITH s_ztipo IN it_r_tipo_doc
    WITH p_zstat EQ iv_status_int
